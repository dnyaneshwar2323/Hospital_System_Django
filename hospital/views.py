...
def home_view(request):
    if request.user.is_authenticated:
        return HttpResponseRedirect('afterlogin') #if user login go to user dashbord
    return render(request, 'index.html') #homeindex
...
def admin_signup_view(request):
    form = forms.AdminSigupForm()  #import form from forms
    if request.method == 'POST':   #If the form has been submitted
        form = forms.AdminSigupForm(request.POST) #form bounded to the post data
        if form.is_valid(): # validation rules pass
            ...
            return HttpResponseRedirect('adminlogin') #redirect after post
    return render(request, 'adminsignup.html', {'form': form}) #form is not valid goto form
...
#Admin  Related View
@login_required(login_url='adminlogin') #used to secure views in your web applications by forcing the client to authenticate with a valid logged-in User.
